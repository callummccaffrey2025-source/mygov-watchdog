...
- **Live navigation (post June-2026 "subtraction" — source of truth is App.tsx)**: 4 tabs — Home, Explore, Learn, Profile. Stack screens: MemberProfile, BillDetail, PartyProfile, TopicBills, BillList, LearnModule, Lesson, WriteToMP, Subscription, About, PrivacyPolicy, Terms, NotificationPreferences, DailyBrief, Activity, Watchlist, Saved, ManageTopics, Methodology, CommunityPostDetail, Council, MatchResult, HypocrisyDetail, ContradictionDetail. Orphaned screens (Polls, Match leaderboard, Stats, Community feed/create, Wallet, Casework, DA Radar, etc.) were deleted 2026-06-10; BallotDecodedScreen kept (legal-gated, currently unwired).
- Home screen (current): VERITY header (Watchlist + Activity icons) → greeting + parliament status → Daily Brief hero card → Your Representative → Have Your Say (bill swipe) → Continue Learning → How [MP] Voted
- MP profiles: real photos, party-coloured headers, stats bar, Overview/Votes/Speeches/More tabs, rebellion indicators, ministerial_role badge, Consistency Index, Donations vs Voting analysis. 'More'/'Votes' tab hooks are gated on activeTab — don't un-gate them (prevents 16-query fan-out on mount).
- Bill detail: plain English summaries, How Parliament Voted section, Personal Impact card (ask-verity), Verity Pro upsell
- Explore tab: universal search, state filter pills, Parties horizontal scroll (Labor/LNP/Liberal/Greens first), Browse by Topic grid, Local Councils. "Verify a Claim" was REMOVED (Verity is not a fact-checker) — VerifyModal code deleted 2026-06-10.
...
- **Dark mode**: two layers, both follow the system scheme. (1) `context/ThemeContext.tsx` COLORS_LIGHT/COLORS_DARK via `useTheme()`. (2) `theme/tokens.ts` — `colors` is a MUTABLE palette swapped by `applyColorScheme(isDark)` in ThemeProvider; components reading `tokenColors` must call `useTheme()` (even unused) to re-render on scheme flips. AppText/Card/Skeleton/EmptyState are theme-aware as of 2026-06-10.
- **Community feed**: DB tables (`community_posts` etc.) and `useCommunityPosts` hooks still exist, but the feed UI was cut in the June subtraction — only CommunityPostDetailScreen remains (reachable from Saved items).
- **Browse by Topic**: 14 topics in ExploreScreen + TopicBillsScreen TOPIC_ICONS
