...
| ~~`revenuecat-webhook` references `"users"` table~~ | ~~HIGH~~ | ~~Resolved in Prompt 14. Fixed to use `user_preferences` table with `user_id` column. Needs redeploy.~~ | Done |
| ~~`send-push-alerts` references dropped tables~~ | ~~HIGH~~ | ~~Resolved in Prompt 14. Fixed: `politicians` → `members`, `user_push_tokens` → `push_tokens`, `entity_type: "politician"` → `"member"`. Still requires APNS secrets for production use.~~ | Done |
